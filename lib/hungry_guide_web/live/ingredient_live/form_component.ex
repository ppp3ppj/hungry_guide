defmodule HungryGuideWeb.IngredientLive.FormComponent do
  ...
  def update(%{ingredient: ingredient} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Inventories.change_ingredient(ingredient))
     end)}
  end

  @impl true
  def handle_event("validate", %{"ingredient" => ingredient_params}, socket) do
    changeset = Inventories.change_ingredient(socket.assigns.ingredient, ingredient_params)
  ...
  defp save_ingredient(socket, :edit, ingredient_params) do
    case Inventories.update_ingredient(socket.assigns.ingredient, ingredient_params) do
      {:ok, ingredient} ->
